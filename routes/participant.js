...
// works
router.delete('/:participantId/delete', checkIfLoggedIn, async (req, res, next) => {
  try {
    if (!mongoose.Types.ObjectId.isValid(req.params.participantId)) {
      res.status(400).json({ message: 'Specified id is not valid' });
      return;
    ...
    if (findUser._id.toString() === findEvent.event.owner._id.toString()) {
      const deleteParticipant = await Participant.findByIdAndDelete(participantId);
      await User.findByIdAndUpdate(participantId, { $pull: { participantEvents: participant._id } }, { new: true });
      await Event.findByIdAndUpdate(eventId, { $pull: { participant: participant._id } }, { new: true });
